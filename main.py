...
async def on_message(msg):
    global data, trivia_asked, question_token
    ...
    if msg.content.lower().startswith('ado '):
        ...
        if msg_content == 'help':
            ...
            try:
                ...
                if len(city) > 3: #means its a city name
                    ...
                    if city_code_lst:
                        ...
                        data = get_data(weather_url, data={'appid': weather_token,
                                                    'id': city_code,
                                                    'units': 'metric'})
        ...
        elif msg_content == 'bad joke' or msg_content == 'bj':
            data = get_data('https://official-joke-api.appspot.com/random_joke')
            print(data)
            e = discord.Embed(title=f'Bad {(data["type"]).capitalize()} Joke', color=discord.Color.red())
            ...
            param = random.choice(['easy','medium','hard'])
            subject = random.choice(list(subject_codes.keys()))
            
            if param:
                res = get_data(base_url, data={'difficulty':param,
